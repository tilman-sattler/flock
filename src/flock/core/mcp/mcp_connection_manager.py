...
from flock.core.exception.mcp.exceptions import FlockMCPServerInitializationException
from flock.core.flock_registry import FlockRegistry
from flock.core.mcp.mcp_flock_default_session import MCPFlockDefaultSession
...
class MCPConnectionManager():
    ...
    async def get_server(
        ...
        server_connection = await self.launch_server(
            server_name=server_name,
            client_session_factory=client_session_factory,
            init_hook=init_hook
        )

        await server_connection.wait_for_initialization()

        if not server_connection.is_healthy():
            error_message = server_connection._error_message or "Unknown error"
            raise FlockMCPServerInitializationException(
                f"MCP-Server: '{server_name}': Failed to initialize. Details: {error_message}",
                server_name=server_name
            )
    ...
    async def get_server_capabilities(self, server_name: str) -> ServerCapabilities | None:
        ...
        """
        server_connection = await self.get_server(
            server_name=server_name,
            client_session_factory=MCPFlockDefaultSession
        )

        return server_connection._server_capabilities if server_connection else None

    async def disconnect_server(self, server_name: str) -> None:
        """
        Description:
          Disconnect from a specific server if this connection is managing it.
        """
        # TODO: logging
        async with self._lock:
            server_connection = self.running_servers.pop(server_name)
        if server_connection:
            # TODO: logging
            server_connection.request_shutdown()
        else:
    ...
    async def disconnect_all(self) -> None:
        """
        Description:
          Disconnect from all servers in self.running_servers
        """
        # TODO: logging
        servers_to_shutdown = []
        async with self._lock:
        ...
        for server_name, server_connection in servers_to_shutdown:
            # TODO: logging
            server_connection.request_shutdown()
